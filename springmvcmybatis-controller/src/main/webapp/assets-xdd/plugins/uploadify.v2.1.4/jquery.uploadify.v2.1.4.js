		...
		jQuery.extend(jQuery.fn,{
			uploadify:function(options) {
				jQuery(this).each(function(){
					...
					cancelImg       : 'uploadify-cancel.png', // The path to the cancel image for the default file queue item container
				...
				jQuery(this).bind("uploadifySelect", {'action': settings.onSelect, 'queueID': settings.queueID}, function(event, ID, fileObj) {
					if (event.data.action(event, ID, fileObj) !== false) {
						...
						jQuery(queue).append('<div id="' + jQuery(this).attr('id') + ID + '" class="uploadifyQueueItem">\
								<div class="cancel">\
									<a href="javascript:jQuery(\'#' + jQuery(this).attr('id') + '\').uploadifyCancel(\'' + ID + '\')"><img id="cancelimg" src="' + settings.cancelImg + '" border="0" /></a>\
								</div>\
								<span class="fileName">' + fileName + ' (' + byteSize + suffix + ')</span><span id="psps" class="percentage"></span>\
				...
				jQuery(this).bind("uploadifyError", {'action': settings.onError}, function(event, ID, fileObj, errorObj) {
					if (event.data.action(event, ID, fileObj, errorObj) !== false) {
						...
						jQuery("#" + jQuery(this).attr('id') + ID).find('.percentage').text(" - " + errorObj.type + " 上传失败！");
				...
				jQuery(this).bind("uploadifyComplete", {'action': settings.onComplete}, function(event, ID, fileObj, response, data) {
					if (event.data.action(event, ID, fileObj, unescape(response), data) !== false) {
						...
						var a=unescape(response);
						var da=JSON.parse(a);
					
						if(da.message!=null){
							jQuery("#" + jQuery(this).attr('id') + ID).find('.percentage').text(' - 上传失败！');
						}else{
							jQuery("#" + jQuery(this).attr('id') + ID).find('.percentage').text(' - 上传成功！');
						}
						/*jQuery("#" + jQuery(this).attr('id') + ID).find('.percentage').text(' - 上传成功！');*/
