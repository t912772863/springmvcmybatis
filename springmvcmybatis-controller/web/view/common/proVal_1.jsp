...
<c:forEach items="${wmProValDtoList}" var="pro" varStatus="cvs">
            ...
            <c:choose>
                ...
                <c:when test="${pro.inputType == 3}">
                    <c:forTokens items="${pro.options}" delims="," var="item" varStatus="vs1">
                        <c:set var="lableVal" value="${fn:split(item, ':')}" />
                        <input type="radio" style="width: 20px;" id="c_box_${pro.id}_${vs1.index}" <c:if test="${vs1.index == 0}">checked="checked"</c:if> name="proValList[${cvs.index}].value" value="${lableVal[0]}"
                        <c:if test="${lableVal[0] eq pro.value}">checked="checked"</c:if>/>
                        <label style="padding-right: 20px;" for="c_box_${pro.id}_${vs1.index}">${lableVal[1]}</label>
                    </c:forTokens>
                </c:when>
                <c:when test="${pro.inputType == 4}">
                    <select class="form-control" name="proValList[${cvs.index}].value">
                        ...
                        <c:forTokens items="${pro.options}" delims="," var="item" varStatus="vs">
                            ...
                            <c:set var="lableVal" value="${fn:split(item, ':')}" />
                            <option value="${lableVal[0]}">${lableVal[1]}</option>
                        </c:forTokens>
                    </select>
                </c:when>
                ...
                <c:otherwise>
                    <input class="form-control" name="proValList[${cvs.index}].value" value="${pro.value}"
